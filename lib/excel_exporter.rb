module PG
  class Result
    def to_excel(options = {})
      ...
      attributes = self.first.keys.map(&:to_sym)
      ...
      headers = columns.map do |column|
        name = (header_names && header_names[column]) ? header_names[column] : column.to_s.humanize
      ...
      body = self.map do |item|
        cols = columns.map { |colname|
          value = item[colname.to_s]
          type = types[colname] || 'String'
  ...
  <Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
   ...
   <Worksheet ss:Name="#{title}">
    <Table ss:ExpandedColumnCount="#{columns.length}" ss:ExpandedRowCount="#{self.count + 1}" x:FullColumns="1" x:FullRows="1" ss:DefaultRowHeight="15">
     #{ col_defs.join("\n") }
     <Row>#{ headers.join("\n") }</Row>
      #{ body.join("\n") }
    </Table>
